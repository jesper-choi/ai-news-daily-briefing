...
**뚜껑을 덮으면** 이것으로도 막히지 않을 수 있다. 그 경우 방법은 root 권한이 필요한
시스템 전역 설정뿐이다:

```bash
sudo pmset -a disablesleep 1   # 뚜껑을 덮어도 안 잠
sudo pmset -a disablesleep 0   # 원래대로
```

이건 **켜둔 동안 계속 유효한 전역 설정**이라 코드에서 자동으로 켜지 않는다. 켜둔 채로
가방에 넣으면 맥이 계속 돌아 발열이 생긴다. 쓸 거면 필요한 동안만 켜고 끄는 걸 권한다.
