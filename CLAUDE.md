...
- **맥이 자면 생성이 깨진다.** 프로세스가 얼어붙고 진행 중이던 HTTPS 연결이 끊긴다.
  요청 타임아웃으로는 못 막는다(멈춘 동안 타이머도 멈춤). 두 겹으로 대응한다:
  `_keep_awake()`가 `caffeinate -i -s`로 막아보고(`-s`는 AC 전원일 때만 유효),
  그래도 자면 `_watch_for_sleep()`이 시계 공백으로 알아채고 생성을 중단한다.
  중단하면 캐시를 저장하지 않아서 다음 주기에 처음부터 다시 만든다 - 이어붙이지 않는
  게 의도다(끊긴 연결로 실패만 쌓이느니 깨어 있을 때 온전히 만드는 게 낫다).
- **생성 대상 날짜는 `service.target_day()`가 정한다.** 7시(=`GENERATE_HOUR`) 전이면
  아직 오늘 것을 만들 때가 아니고 자정은 지났으므로 대상은 **어제**다. 며칠 만에 새벽
  2시에 맥을 켜면 어제 것을 만들고, 오늘 것은 7시 이후에 만든다. 그보다 오래된 날은
  대상이 아니다 - 통째로 안 켠 날은 그냥 지나간다(그날 프론트 페이지는 이미 사라졌다).
  자정에 돌리면 맥이 자는 시간과 겹쳐 생성이 계속 끊겼던 게 이 규칙의 이유.
- **날짜는 시작할 때 정해서 끝까지 들고 간다**(`_build_data(day)`). 23:55에 시작해
  00:10에 끝나면 `date.today()`가 다음날이 되어 엉뚱한 날짜로 저장된다.
